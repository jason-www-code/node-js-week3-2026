...
const members = JSON.parse(JSON.stringify(initialMembers));
// 2. 下一個新增會員要使用的 id
...
function validateBody(body) {
  ...
  return isValidate
    ? { valid: true }
    : { valid: false, error: "缺 name 或 level" };
}
...
router.get("/", (req, res) => {
  const memberList = filterByQuery(members, req.query);
...
router.get("/:id", (req, res) => {
  ...
  if (findMember) return res.status(200).json(findMember);
...
router.put("/:id", (req, res) => {
  ...
  if (memberIndex === -1) return res.status(404).json({ error: "會員不存在" });
